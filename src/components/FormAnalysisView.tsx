...
function FormBuddyQaAssistant({ analysisData }: { analysisData: AnalyzeFormApiResponse }) {
  const [question, setQuestion] = useState('');
  const [loading, setLoading] = useState(false);
  const [chatHistory, setChatHistory] = useState<Array<{ question: string; answer: string }>>([
    {
      question: 'What is this form used for?',
      answer: renderSafeString(analysisData.summary) || 'This is an official document form.'
    }
  ]);

  const handleAskQuestion = async (qToAsk?: string) => {
    const targetQ = (qToAsk || question).trim();
    ...
    try {
      ...
      if (data.success && data.answer) {
        setChatHistory(prev => [...prev, { question: targetQ, answer: data.answer }]);
      } else {
        setChatHistory(prev => [...prev, { question: targetQ, answer: data.error || 'Unable to process question. Please try again.' }]);
      }
    } catch (err: any) {
      setChatHistory(prev => [...prev, { question: targetQ, answer: 'Network connection failed while asking question.' }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const suggestedQuestions = [
    ...
    'What are key mistakes to avoid?',
    'Can I sign in blue ink?',
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
      <div className="flex items-center gap-2 pb-3 border-b border-slate-100 dark:border-slate-800">
        <div className="w-8 h-8 rounded-xl bg-blue-100 text-blue-600 dark:bg-blue-950 dark:text-blue-400 flex items-center justify-center font-bold text-xs shadow-xs">
          <MessageSquare className="w-4 h-4" />
        </div>
        <div>
          <h3 className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-1.5">
            Ask FormBuddy AI
            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-50 text-blue-600 dark:bg-blue-950/80 dark:text-blue-300 border border-blue-200 dark:border-blue-800 flex items-center gap-1">
              <Sparkles className="w-2.5 h-2.5" /> Q&A
            </span>
          </h3>
          <p className="text-[11px] text-slate-500 dark:text-slate-400">
            Ask any question about filling this form
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3 max-h-[260px] overflow-y-auto pr-1 text-xs">
        {chatHistory.map((item, idx) => (
          <div key={idx} className="space-y-1.5">
            <div className="flex justify-end">
              <div className="bg-blue-600 text-white px-3.5 py-2 rounded-2xl rounded-tr-none max-w-[85%] font-medium shadow-xs">
                {item.question}
              </div>
            </div>
            <div className="flex justify-start">
              <div className="bg-slate-50 dark:bg-slate-800/90 text-slate-800 dark:text-slate-200 px-3.5 py-2.5 rounded-2xl rounded-tl-none max-w-[90%] border border-slate-200/80 dark:border-slate-700/80 leading-relaxed shadow-xs">
                {item.answer}
              </div>
            </div>
          </div>
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-slate-100 dark:bg-slate-800 text-slate-500 px-3 py-2 rounded-2xl animate-pulse text-[11px] flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-blue-500 animate-spin" /> FormBuddy is answering...
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-1.5 pt-1">
        <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Suggested Questions:</span>
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
          {suggestedQuestions.map((sq, i) => (
            <button
              ...
              disabled={loading}
              className="shrink-0 px-2.5 py-1 rounded-full text-[10px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-blue-950 hover:text-blue-600 dark:hover:text-blue-400 border border-slate-200 dark:border-slate-700 transition-colors"
            >
              {sq}
            </button>
          ))}
      ...
      >
        <input
          ...
          disabled={loading}
          className="flex-grow px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white text-xs placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
        />
        <button
          type="submit"
          disabled={loading || !question.trim()}
          className="px-3.5 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-semibold rounded-xl transition-colors shrink-0 flex items-center justify-center"
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-4 lg:sticky lg:top-24">
          ...
          {/* Interactive FormBuddy Q&A Assistant */}
          <FormBuddyQaAssistant analysisData={analysisData} />
        </div>
